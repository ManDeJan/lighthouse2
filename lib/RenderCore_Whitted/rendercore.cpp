...
#define AMBIENTLIGHT 0.01f 
#define MAXDEPTH 5 // Maximum recursion depth for each ray.
#define NOISE_PROB 0.1f // To increase speed, we only shoot a primary ray for 10% of the pixels.
...
float3 RenderCore::calcRayColor(Ray ray, uint depth) {
	...
	switch (m.type) {
		case MaterialType::MIRROR: {
			...
			return AMBIENTLIGHT * m.color + (1 - AMBIENTLIGHT) * calcRayColor(reflectiveRay, ++depth);
		}
		...
		case MaterialType::LIGHT: {
			...
			if (backFacing)
				return make_float3(AMBIENTLIGHT);
			return m.color;
		}

		case MaterialType::DIFFUSE: {
			return fmaxf(m.color * AMBIENTLIGHT, calcLightContributions(m.color, i.location, triangleNormal, i.distance));
		}
...
float3 RenderCore::calcLightContributions(float3 mColor, float3 iLocation, float3 triangleN, float closestIntersection) {
	...
	*/
	...
	for (CoreLightTri light : areaLights) {
		...
		float3 randomPoint = (1 - sqrtf(u)) * light.vertex0 + sqrtf(u) * (1 - v) * light.vertex1 + sqrtf(u) * v * light.vertex2;
		...
		if (dot(triangleN, sRayDir) > 0.0f && dot(light.N, -sRayDir) > 0.0f)
		{
			...
			if (!occluded)
			{
				float3 scaledColor = mColor * light.radiance * (1.0f / (distToLight * distToLight)) *dot(light.N, -normalizedSRayDir);
				litColor += scaledColor * dot(triangleN, normalizedSRayDir); //Lambertian Shading
			}
		}
	}
...
//  +-----------------------------------------------------------------------------+
void RenderCore::SetLights(const CoreLightTri *_areaLights, const int areaLightCount,
						   const CorePointLight *_pointLights, const int pointLightCount,
						   const CoreSpotLight *_spotLights, const int spotLightCount,
						   const CoreDirectionalLight *_directionalLights, const int directionalLightCount)
{
