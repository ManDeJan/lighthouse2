...
void Node::partition() {
    int split = count / 2;
    nodes[nodeIndex + 1] = Node(first(), split); //left node
    nodes[nodeIndex + 2] = Node(split, count);   //right node
...
AABB BVH::calculateBounds(int first, int count) {
    ...
    for (int i = first; i <= count; i++) { primitives[i]; }
    return AABB(x, y, z, xd, yd, zd);
}
} // namespace lh2core
